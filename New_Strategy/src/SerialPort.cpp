...
namespace LibSerial 
{
    ...
    bool SerialPort::Send_Velocities(int channel, float command_left, float command_right)
    {
        float left_vel = command_left, right_vel = command_right;

        if(SerialPort::IsOpen()){
            //Inicializamos o vetor de bytes a ser transferido
            char vector_bytes[13];
            //QByteArray bytes(13, 0x0);
            vector_bytes[0] = 18;
            //Setamos o byte 1 como o número do robô selecionado
            vector_bytes[1] = (char)channel;
            vector_bytes[12] = 19;

            //Criamos uma variável para converter a soma dos bytes de velocidade
            Short2Char cont;
            cont.Short = 0;
            cont.Short += vector_bytes[0] + vector_bytes[1];
            ...
            vector_bytes[2] = valor1.Bytes[0];
            vector_bytes[3] = valor1.Bytes[1];
            vector_bytes[4] = valor1.Bytes[2];
            vector_bytes[5] = valor1.Bytes[3];
            ...
            vector_bytes[6] = valor2.Bytes[0];
            vector_bytes[7] = valor2.Bytes[1];
            vector_bytes[8] = valor2.Bytes[2];
            vector_bytes[9] = valor2.Bytes[3];
            ...
            vector_bytes[10] = cont.Bytes[0];
            vector_bytes[11] = cont.Bytes[1];
