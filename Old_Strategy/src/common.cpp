...
namespace common{
    ...
    double ajusta_angulo(double angle){
        if (angle < -180)
            angle = angle + 360;
        else if (angle > 180)
            angle = angle - 360;
    ...
    State Global_State2State(vss_state::Global_State global_state, string main_color){
        ...
        if(main_color == "yellow"){
            for(int i = 0 ; i < 3 ; i++){
                ...
                // Blue Robots POSE
                state.robots[i+3].pose.x =  global_state.robots_blue(i).pose().x();
                //cout << endl << " Global_sta: " << global_state.robots_blue(i).pose().x() << endl;           // Pos X
